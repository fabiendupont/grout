...
bool mcast_should_forward(
	...
	struct mcast_snooping *mcast = bridge_get_mcast_snooping(bridge);
	if (mcast == NULL || !mcast->igmp_enabled)
		return true; // snooping disabled, flood

	struct mdb_entry *entry = mdb_lookup(mcast, dst_mac);
	if (entry == NULL)
		return true; // no MDB entry, flood

	// Check if iface_id is in the entry's port list
	for (uint16_t p = 0; p < gr_vec_len(entry->member_ports); p++) {
		if (entry->member_ports[p] == iface_id)
			return true;
	}

	return false; // port not subscribed to this group
}
...
struct mdb_entry *mdb_lookup(struct mcast_snooping *mcast, const struct rte_ether_addr *group_mac) {
	void *data;
...
int igmp_process_report(
	...
	if ((ip_version == 4 && !mcast->igmp_enabled) || (ip_version == 6 && !mcast->mld_enabled))
		return 0;
...
static struct api_out mdb_add_cb(const void *request, struct api_ctx *) {
	...
	int ret = mdb_add_entry(
		mcast, &req->group_mac, &req->group_ip, req->ip_version, req->iface_id, true
	);
