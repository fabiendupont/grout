...
uint16_t port_mirror_get_dest(uint16_t bridge_id, uint16_t iface_id, uint8_t direction) {
	if (bridge_id >= L2_MAX_BRIDGES)
		return GR_IFACE_ID_UNDEF;

	struct port_mirroring *pm = &port_mirrors[bridge_id];

	for (uint16_t s = 0; s < MAX_MIRROR_SESSIONS; s++) {
		struct mirror_session *session = &pm->sessions[s];
		if (!session->enabled)
			continue;
		if (!(session->direction & direction))
			continue;
		if (port_mirror_is_source(session, iface_id))
			return session->dest_port;
	}
