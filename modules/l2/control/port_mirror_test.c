...
static void test_session_set_get(void **state) {
	...
	assert_int_equal(
		port_mirror_session_set(0, 1, true, sources, 3, 50, GR_MIRROR_DIR_BOTH, false, 0), 0
	);
...
static void test_session_del(void **state) {
	...
	port_mirror_session_set(0, 2, true, sources, 1, 50, GR_MIRROR_DIR_INGRESS, false, 0);
...
static void test_session_rspan(void **state) {
	...
	assert_int_equal(
		port_mirror_session_set(
			0, 3, true, sources, 1, 50, GR_MIRROR_DIR_INGRESS, true, 100
		),
		0
	);
...
static void test_should_mirror(void **state) {
	...
	port_mirror_session_set(1, 1, true, sources, 2, 50, GR_MIRROR_DIR_INGRESS, false, 0);

	assert_true(port_mirror_should_mirror(1, 10, GR_MIRROR_DIR_INGRESS, session_ids, &num));
	assert_int_equal(num, 1);
	assert_int_equal(session_ids[0], 1);

	// Wrong direction
	assert_false(port_mirror_should_mirror(1, 10, GR_MIRROR_DIR_EGRESS, session_ids, &num));

	// Not a source port
	assert_false(port_mirror_should_mirror(1, 99, GR_MIRROR_DIR_INGRESS, session_ids, &num));
...
static void test_filter_set(void **state) {
	...
	port_mirror_session_set(2, 1, true, sources, 1, 50, GR_MIRROR_DIR_BOTH, false, 0);

	assert_int_equal(port_mirror_filter_set(2, 1, true, vlans, 2, 0x0800, &mac, NULL), 0);
...
static void test_invalid_params(void **state) {
	...
	assert_int_equal(
		port_mirror_session_set(
			L2_MAX_BRIDGES, 1, true, sources, 1, 50, GR_MIRROR_DIR_BOTH, false, 0
		),
		-EINVAL
	);

	assert_int_equal(
		port_mirror_session_set(0, 0, true, sources, 1, 50, GR_MIRROR_DIR_BOTH, false, 0),
		-EINVAL
	);
