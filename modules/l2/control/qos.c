...
static const uint8_t default_dscp_to_cos[64] = {
	[0] = 0,  [8] = 1,  [10] = 1, [12] = 1, [14] = 1, [16] = 2, [18] = 2, [20] = 2,
	[22] = 2, [24] = 3, [26] = 3, [28] = 3, [30] = 3, [32] = 4, [34] = 4, [36] = 4,
	[38] = 4, [40] = 5, [44] = 5, [46] = 6, [48] = 6, [56] = 7,
};
...
bool qos_meter_packet(uint16_t iface_id, uint16_t lcore_id, uint8_t priority, uint32_t packet_len) {
	...
	if (cfg->queues[priority].rate_limit_kbps > 0) {
		...
		if (qos_meter_check(&qs->meter, &qs->profile, tsc, packet_len) == RTE_COLOR_RED) {
...
static struct api_out qos_queue_set_cb(const void *request, struct api_ctx *) {
	...
	int ret = qos_queue_set(
		req->iface_id, req->priority, req->rate_limit_kbps, req->weight, req->min_rate_kbps
	);
	return api_out(ret < 0 ? -ret : 0, 0, NULL);
}
