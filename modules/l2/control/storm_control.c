...
static int
init_meter_profile(struct rte_meter_trtcm_profile *profile, uint64_t rate_kbps, bool use_pps) {
...
bool storm_control_meter_packet(
	...
	if (cfg->shutdown_on_violation && *violations >= cfg->violation_threshold) {
		...
		LOG(WARNING,
		    ...
		    iface_id,
		    traffic_type == STORM_TRAFFIC_BROADCAST ?
			    "broadcast" :
			    traffic_type == STORM_TRAFFIC_MULTICAST ?
			    "multicast" :
			    "unknown-unicast");
	}
...
static struct api_out storm_control_set_cb(const void *request, struct api_ctx *) {
	...
	int ret = storm_control_set_config(
		...
		req->bcast_rate_kbps,
		req->mcast_rate_kbps,
		req->unknown_uc_rate_kbps,
		req->use_pps,
		req->shutdown_on_violation,
		req->violation_threshold
	);
