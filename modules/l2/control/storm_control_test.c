...
extern int __real_rte_meter_trtcm_profile_config(
	...
	const struct rte_meter_trtcm_params *
);
extern int
__real_rte_meter_trtcm_config(struct rte_meter_trtcm *, const struct rte_meter_trtcm_profile *);
...
static void test_config(void **state) {
	...
	assert_int_equal(storm_control_set_config(100, true, 1000, 500, 250, false, true, 5), 0);
	...
	assert_int_equal(
		storm_control_set_config(L2_MAX_IFACES, true, 0, 0, 0, false, false, 5), -EINVAL
	);
