...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		...
		with_help("Query interval (seconds).", ec_node_uint("INTERVAL", 1, 65535, 10)),
		with_help("Max response time (1/10 seconds).", ec_node_uint("RESP", 1, 65535, 10)),
		with_help("MDB aging time (seconds).", ec_node_uint("AGING", 1, UINT32_MAX, 10))
	);
	if (ret < 0)
		return ret;
