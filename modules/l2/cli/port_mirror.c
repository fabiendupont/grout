...
static const char *dir_str(uint8_t dir) {
	switch (dir) {
	case GR_MIRROR_DIR_INGRESS:
		return "ingress";
	case GR_MIRROR_DIR_EGRESS:
		return "egress";
	case GR_MIRROR_DIR_BOTH:
		return "both";
	default:
		return "unknown";
	}
...
#define MIRROR_CTX(root) CLI_CONTEXT(root, CTX_ARG("port-mirror", "Port mirroring configuration."))
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		...
		with_help(
			"Comma-separated source interfaces.",
			ec_node_re("SOURCES", "[a-zA-Z0-9_,]+")
		),
