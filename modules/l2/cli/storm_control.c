...
#define STORM_CTX(root) CLI_CONTEXT(root, CTX_ARG("storm-control", "Storm control configuration."))
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		...
		with_help(
			"Broadcast rate limit (kbps or pps).",
			ec_node_uint("BCAST", 0, UINT64_MAX, 10)
		),
		with_help("Multicast rate limit.", ec_node_uint("MCAST", 0, UINT64_MAX, 10)),
		with_help(
			"Unknown unicast rate limit.", ec_node_uint("UNKNOWN", 0, UINT64_MAX, 10)
		),
		with_help("Violation threshold (1-255).", ec_node_uint("THRESHOLD", 1, 255, 10)),
